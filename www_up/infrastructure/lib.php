...
function generateID(): string {
    // If GMP is available, use it for true base62 encoding
    if (function_exists('gmp_import')) {
        ...
        return str_pad($result, 22, '0', STR_PAD_LEFT);
    }
    
    // Fallback: Generate random base62 string directly
    $base62 = '0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ';
    $result = '';
    for ($i = 0; $i < 22; $i++) {
        $result .= $base62[random_int(0, 61)];
    }
    return $result;
}
...
/**
 * Check authentication from session cookie and return user ID
 */
function checkAuth(): ?string {
    $token = $_COOKIE['aiofc_session'] ?? '';
    return getUserIdFromToken($token, true); // Refresh expiry on each use
}
